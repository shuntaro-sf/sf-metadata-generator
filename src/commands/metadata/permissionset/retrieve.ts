...
const messages = Messages.loadMessages('@shuntaro/sf-metadata-generator', 'permissionset.retrieve');

export type PermissionsetRetrieveResult = {
  csvDataStr: { [key: string]: string };
};

export default class Retrieve extends SfCommand<PermissionsetRetrieveResult> {
  ...
  private static header = ConfigData.permissionsetRetrieveConfig.header;
  private static permissionsetExtension = ConfigData.permissionsetRetrieveConfig.permissionsetExtension;
  private static metaJson = [] as Array<{ [key: string]: any }>;

  public async run(): Promise<PermissionsetRetrieveResult> {
    ...
    Object.keys(metaJsons).forEach((fullName) => {
      ...
      const objectConverter = new PermissionsetConvert();
      ...
      if (Retrieve.metaJson.length > 0) {
        ...
        writeFileSync(join(flags.outputdir, fullName + '.permissionset-meta.csv'), csvStr, 'utf8');
      }
    });
    this.log();
  ...
  private getFullNameFromPath(path: string): string {
    const fullNameRegExp = new RegExp(Retrieve.permissionsetExtension + '$');
    const fullNameMatch = parse(path).base.match(fullNameRegExp);
    if (fullNameMatch === null) {
      return '';
    }
    return parse(path).base.replace(Retrieve.permissionsetExtension, '');
  }
  ...
  private async retrieve(
    ...
    for (const file of metadataDir.files) {
      if (extname(file.path) === Retrieve.permissionsetExtension) {
    ...
    Object.keys(fileBuffers).forEach((fullName) => {
      parser.parseString(fileBuffers[fullName], (err, metaJson) => {
        if (err) {
          ...
          metaJsons[fullName] = metaJson.permissionset;
        }
      });
    });
    return metaJsons;
