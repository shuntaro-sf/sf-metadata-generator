...
const messages = Messages.loadMessages('@shuntaro/sf-metadata-generator', 'permissionset.generate');

export type PermissionsetGenerateResult = {
  MetaJson: Json;
};

export default class Generate extends SfCommand<PermissionsetGenerateResult> {
  ...
  public static readonly flags = {
    ...
    delimiter: Flags.string({
      ...
      default: ConfigData.permissionsetGenerateConfig.delimiter,
    }),
  };

  private static xmlSetting = ConfigData.objectGenerateConfig.xmlSetting as { [key: string]: string };
  private static delimiter = ConfigData.permissionsetGenerateConfig.delimiter;
  private static permissionTags = ConfigData.permissionsetGenerateConfig.permissionTags as Json;
  private static options = ConfigData.permissionsetGenerateConfig.options as Json;
  private static indentationLength = ConfigData.permissionsetGenerateConfig.indentationLength;
  private static permissionsetExtension = ConfigData.permissionsetGenerateConfig.permissionsetExtension;
  ...
  public async run(): Promise<PermissionsetGenerateResult> {
    ...
    Generate.successResults.push({
      FULLNAME: fullName + Generate.permissionsetExtension,
      PATH: join(flags.outputdir, fullName + Generate.permissionsetExtension).replace('//', '/'),
    });
  ...
  private getFullNameFromPath(path: string): string {
    const fullNameRegExp = new RegExp(Generate.permissionsetExtension + '$');
    const fullNameMatch = parse(path).base.match(fullNameRegExp);
    if (fullNameMatch === null) {
      return '';
    }
    return parse(path).base.replace(Generate.permissionsetExtension, '');
  }
  ...
  private saveMetaData(
    ...
    Object.keys(Generate.metaXmls).forEach((fullName) => {
      if (existsSync(join(flags.outputdir, fullName + '.' + Generate.permissionsetExtension))) {
        // for creating
        writeFileSync(
          join(flags.outputdir, fullName + '.' + Generate.permissionsetExtension),
